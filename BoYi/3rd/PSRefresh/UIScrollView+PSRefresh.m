...
//自定义一个右拉跳转
- (void)addRefreshLeftWithClosure:(PSRefreshClosure)closure {
    if (!self.footer && !self.gifFooter) {
        self.footer = [PSNormalRefreshFooter footer];
        [self.footer setTitle:@"滑动查看图文详情" forState:PSRefreshStatePullCanRefresh];
        [self.footer setTitle:@"释放查看图文详情" forState:PSRefreshStateReleaseCanRefresh];
        [self.footer setTitle:@"正在加载图文详情" forState:PSRefreshStateRefreshing];
        [self.footer setTitle:@"没有图文详情" forState:PSRefreshStateNoMoreData];
        [self.footer setTextColor:RGBA(102, 102, 102, 1)];
        self.footer.imageView.image = [UIImage imageNamed:@"物质-右拉"];
