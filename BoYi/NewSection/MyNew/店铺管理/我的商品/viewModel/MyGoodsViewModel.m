...
@implementation MyGoodsViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
	...
	[self.dataArray addObjectsFromArray:[MyGoodsModel mj_objectArrayWithKeyValuesArray:object]];
}
...
- (RACCommand *)refreshDataCommand {
	...
	if (!_refreshDataCommand) {
		...
		_refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
			...
			@strongify(self);
			return [[RequestManager sharedManager] RACRequestUrl:URL_myGoodsList
														  method:POST
														  loding:@"加载中..."
															 dic:input];
		}];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	MyGoodsTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
	if (!cell) {
		cell = [[MyGoodsTableViewCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:@"cell"];
	}
