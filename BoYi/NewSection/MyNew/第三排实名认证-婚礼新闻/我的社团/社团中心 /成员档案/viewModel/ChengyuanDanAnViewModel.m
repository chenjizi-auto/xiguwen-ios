...
@implementation ChengyuanDanAnViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
    ...
    [self.dataArray addObjectsFromArray:[ChengyuanDanAnModel mj_objectArrayWithKeyValuesArray:object]];
...
- (RACCommand *)refreshDataCommand {
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            @strongify(self);
            return [[RequestManager sharedManager] RACRequestUrl:URL_associationsCenterMemberScheduleList
                                                          method:POST
                                                          loding:@"加载中..."
                                                             dic:input];
        }];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
	UIView *sectionView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, ScreenWidth, 30.0f)];
	[sectionView setBackgroundColor:UIColorFromRGB(0xF0F0F0)];
	UILabel *numberLabel = [[UILabel alloc] init];
	[numberLabel setFont:[UIFont systemFontOfSize:12.0f]];
	[numberLabel setTextColor:UIColorFromRGB(0x898989)];
	[numberLabel setText:[NSString stringWithFormat:@"全部成员（%ld）",self.dataArray.count]];
	[sectionView addSubview:numberLabel];
	numberLabel.sd_layout
	.centerYEqualToView(sectionView)
	.leftSpaceToView(sectionView, 16.0f)
	.rightSpaceToView(sectionView, 16.0f)
	.heightIs(20.0f);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ChengyuanDanAnTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ChengyuanDanAnTableViewCell"];
