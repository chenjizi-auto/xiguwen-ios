//
//  YaoQingNewchengyuanViewModel.m
//  BoYi
//
...
@implementation YaoQingNewchengyuanViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
    ...
    [self.dataArray addObjectsFromArray:[YaoQingNewchengyuanModel mj_objectArrayWithKeyValuesArray:object]];
}
...
- (RACSubject *)inviteItemSubject {
	
	if (!_inviteItemSubject) _inviteItemSubject = [RACSubject subject];
	
	return _inviteItemSubject;
}
...
- (RACCommand *)refreshDataCommand {
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            @strongify(self);
            return [[RequestManager sharedManager] RACRequestUrl:URL_associationsCenterInviteMemberList
                                                          method:POST
                                                          loding:@"加载中..."
                                                             dic:input];
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    YaoQingNewchengyuanTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"YaoQingNewchengyuanTableViewCell"];
    cell.model = self.dataArray[indexPath.row];
	WeakSelf(self);
	[cell setOnDidinvite:^{
		[weakSelf.inviteItemSubject sendNext:weakSelf.dataArray[indexPath.row]];
	}];
