...
@implementation MyXuqiuViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
    ...
    [self.dataArray addObjectsFromArray:[MyXuqiuModel mj_objectArrayWithKeyValuesArray:object]];
...
- (RACSubject *)editItemSubject {
	if (!_editItemSubject) {
		_editItemSubject = [RACSubject subject];
	}
	return _editItemSubject;
}

- (RACSubject *)closeItemSubject {
	if (!_closeItemSubject) {
		_closeItemSubject = [RACSubject subject];
	}
	return _closeItemSubject;
}
...
- (RACCommand *)refreshDataCommand {
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            @strongify(self);
            return [[RequestManager sharedManager] RACRequestUrl:URL_myDemandList
                                                          method:POST
                                                          loding:@"加载中..."
                                                             dic:input];
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MyXuqiuTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MyXuqiuTableViewCell"];
	...
	[[[cell.editBtn rac_signalForControlEvents:UIControlEventTouchUpInside] takeUntil:cell.rac_prepareForReuseSignal] subscribeNext:^(__kindof UIControl * _Nullable x) {
		[weakSelf.editItemSubject sendNext:weakSelf.dataArray[indexPath.row]];
	...
	[[[cell.closeBtn rac_signalForControlEvents:UIControlEventTouchUpInside] takeUntil:cell.rac_prepareForReuseSignal] subscribeNext:^(__kindof UIControl * _Nullable x) {
		[weakSelf.closeItemSubject sendNext:weakSelf.dataArray[indexPath.row]];
	}];
