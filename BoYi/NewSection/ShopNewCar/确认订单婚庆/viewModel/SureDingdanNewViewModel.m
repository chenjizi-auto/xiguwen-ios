...
#import "SureDingdanHeader.h"
#import "SureDingdanHQFootView.h"
@implementation SureDingdanNewViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
    ...
    [self.dataArray addObjectsFromArray:[Cartlist mj_objectArrayWithKeyValuesArray:object[@"cartlist"]]];
    
    for (int i = 0; i < self.dataArray.count; i++) {
        self.dataArray[i].liulan = @"";
    }
    self.model = [SureDingdanNewModel mj_objectWithKeyValues:object];
...
- (RACCommand *)refreshDataCommand {
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            NSString *url;
            if (self.type == 1) {
                url = [HOMEURL stringByAppendingString:@"appapi/ordershq/getweddingmoney"];
            }else {
                url = [HOMEURL stringByAppendingString:@"appapi/carthq/confirm_orders"];
            }
            @strongify(self);
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    SureDingdanHeader *header = [[NSBundle mainBundle]loadNibNamed:@"SureDingdanHeader" owner:nil options:nil].firstObject;
...
- (UIView *)tableView:(UITableView *)tableView viewForFooterInSection:(NSInteger)section {
    ...
    SureDingdanHQFootView *header = [[NSBundle mainBundle]loadNibNamed:@"SureDingdanHQFootView" owner:nil options:nil].firstObject;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SureDingdanNewTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"SureDingdanNewTableViewCell"];
...
- (void)textFieldDidEndEditing:(UITextField *)textField
{
    ...
    self.dataArray[index].liulan = textField.text;
}
