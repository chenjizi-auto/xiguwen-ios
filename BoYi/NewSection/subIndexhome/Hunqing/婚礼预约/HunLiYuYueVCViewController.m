...
@interface HunLiYuYueVCViewController ()

@property (weak, nonatomic) IBOutlet UITableView *table;
@property (strong,nonatomic) HunLiYuYueVCViewModel *viewModel;
@property (strong,nonatomic) ShareNewmodel *sharemodel;
@end

@implementation HunLiYuYueVCViewController
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.title = @"婚礼预约";
    [self addPopBackBtn];
    [self cellClick];
    [self setupTableView];
...
- (void)shareData{
    NSDictionary *dic = @{@"id":@(self.guanggaoID),@"type":@"2"};
...
- (void)setupTableView {
    ...
    [self.table registerNib:[UINib nibWithNibName:@"HunLiYuYueVCTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"HunLiYuYueVCTableViewCell"];
//    [self.table registerNib:[UINib nibWithNibName:@"" bundle:[NSBundle mainBundle]] forHeaderFooterViewReuseIdentifier:@""];
    ...
    self.table.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        ...
        //传入参数 进行刷新
        //        guanggaoID
        [self.viewModel.refreshDataCommand execute:@{@"id":@"1"}];
    }];
...
//初始化viewModel
- (HunLiYuYueVCViewModel *)viewModel {
    if (!_viewModel) {
        _viewModel = [[HunLiYuYueVCViewModel alloc] init];
    }
    return _viewModel;
}
