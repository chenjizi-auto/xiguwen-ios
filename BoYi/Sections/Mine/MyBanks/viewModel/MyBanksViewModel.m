...
@implementation MyBanksViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
    ...
    [self.dataArray addObjectsFromArray:[GetMoneyModel mj_objectArrayWithKeyValuesArray:object]];
...
- (RACCommand *)refreshDataCommand {
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            @strongify(self);
            return [[RequestManager sharedManager] RACRequestUrl:URL_FIND_ALL_CARD
                                                          method:POST
                                                          loding:@"加载中..."
                                                             dic:input];
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MyBanksTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MyBanksTableViewCell"];
