...
@implementation MyCaseViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
    ...
    [self.dataArray addObjectsFromArray:[MyCaseModel mj_objectArrayWithKeyValuesArray:object]];
...
- (RACCommand *)refreshDataCommand {
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            @strongify(self);
            return [[RequestManager sharedManager] RACRequestUrl:URL_GET_MY_CASE
                                                          method:POST
                                                          loding:@"加载中..."
                                                             dic:input];
        }];
...
- (RACCommand *)updateExampleViewCommand {
    
    if (!_updateExampleViewCommand) {
//        @weakify(self);
        _updateExampleViewCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            
//            @strongify(self);
            return [[RequestManager sharedManager] RACRequestUrl:URL_updateExampleView
                                                          method:POST
                                                          loding:@"加载中..."
                                                             dic:input];
        }];
    }
    return _updateExampleViewCommand;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MyCaseTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"MyCaseTableViewCell"];
