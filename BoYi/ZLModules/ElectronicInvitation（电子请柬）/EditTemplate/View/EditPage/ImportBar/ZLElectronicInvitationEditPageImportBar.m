//
//  ZLElectronicInvitationEditPageImportBar.m
//  ProjectModules
//
...
@interface ZLElectronicInvitationEditPageImportBar ()<UITextViewDelegate>
...
@implementation ZLElectronicInvitationEditPageImportBar
...
- (void)keyboardDidChangeFrame:(NSNotification *)notification {
    ...
    //找到外部的输入框并进行引用
    if ([[UIResponder currentFirstResponder] isKindOfClass:[ZLElectronicInvitationEditTemplateTextView class]]) {
...
- (void)sureButtonAction {
    if (![self.currentTextView.text isEqualToString:self.textView.text]) {
        if (self.delegate && [self.delegate respondsToSelector:@selector(saveTextWithCurrentTextView:LatestText:)]) {
            [self.delegate saveTextWithCurrentTextView:self.currentTextView LatestText:self.textView.text];
        }
    }
