//
//  ZLElectronicInvitationGuestsReplyView.m
//  ProjectModules
//
...
#import "ZLElectronicInvitationGuestsReplyStaticCell.h"
#import <UIButton+AFNetworking.h>
#import <MJRefresh.h>

@interface ZLElectronicInvitationGuestsReplyView ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation ZLElectronicInvitationGuestsReplyView
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        tableView.contentInset = UIEdgeInsetsMake(statusBarHeight + 44.0 + CGRectGetHeight(self.functionBar.frame), 0, statusBarHeight == 20.0 ? 0 : 14.0, 0);
        [tableView registerClass:[ZLElectronicInvitationGuestsReplyCell class] forCellReuseIdentifier:@"ZLElectronicInvitationGuestsReplyCell"];
        [tableView registerClass:[ZLElectronicInvitationGuestsReplyStaticCell class] forCellReuseIdentifier:@"ZLElectronicInvitationGuestsReplyStaticCell"];
...
- (UIView *)functionBar {
    if (!_functionBar) {
        ...
        CGFloat maxCount = self.infoModel.sectionModels.count > 3 ? 3 : self.infoModel.sectionModels.count;
        ...
        for (NSInteger index = 0; index < _infoModel.sectionModels.count; index++) {
            ...
            if (index == self.infoModel.currentSection) {
                item.selected = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (!self.infoModel.sectionModels[self.infoModel.currentSection].showStaticPage) {        
        return [ZLElectronicInvitationGuestsReplyCell reuseCellWithTableView:tableView IndexPath:indexPath InfoModel:self.infoModel];
    }
    ZLElectronicInvitationGuestsReplyStaticCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ZLElectronicInvitationGuestsReplyStaticCell" forIndexPath:indexPath];
    return cell;
}
...
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath {
    if (self.deleteData) {
        [self showDeleteDataHudView];
        self.infoModel.willDeleteModel = self.infoModel.sectionModels[self.infoModel.currentSection].unitModels[indexPath.row];
        self.deleteData();
    }
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    CGFloat statusBarHeight = [[UIApplication sharedApplication] statusBarFrame].size.height;
    return !self.infoModel.sectionModels[self.infoModel.currentSection].showStaticPage
        ? self.infoModel.sectionModels[self.infoModel.currentSection].unitModels[indexPath.row].cellHeight
