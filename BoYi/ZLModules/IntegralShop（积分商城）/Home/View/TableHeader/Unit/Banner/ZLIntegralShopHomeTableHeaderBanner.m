...
- (void)setImageUrlsArray:(NSArray *)imageUrlsArray {
    ...
    for (NSInteger index = 0; index < count; index++) {
        ...
        [sender setImageForState:UIControlStateNormal withURL:[NSURL URLWithString:imageUrlsArray[[indexArray[index] integerValue]]] placeholderImage:[UIImage imageNamed:@"rectangle_ placeholder"]];
...
- (void)changeOffsetWithScroll:(UIScrollView *)scrollView {
    ...
    if (offsetX == scrollView.frame.size.width) {
        ...
        [sender setImageForState:UIControlStateNormal withURL:[NSURL URLWithString:_imageUrls[self.pageIndex]] placeholderImage:[UIImage imageNamed:@"rectangle_ placeholder"]];
        
        sender = self.pageArrayM[1];
        NSInteger index = self.pageIndex - 1 < 0 ? _imageUrls.count - 1 : self.pageIndex - 1;
        [sender setImageForState:UIControlStateNormal withURL:[NSURL URLWithString:_imageUrls[index]] placeholderImage:[UIImage imageNamed:@"rectangle_ placeholder"]];
