...
@interface ZLIntegralGoodsListCell ()

//左边块
@property (nonatomic,weak) ZLIntegralGoodsListUnitView *leftBlockView;
//右边块
@property (nonatomic,weak) ZLIntegralGoodsListUnitView *rightBlockView;
///顶部的线
@property (nonatomic,weak) CALayer *topLine;
///底部的线
@property (nonatomic,weak) CALayer *bottomLine;
///
@property (nonatomic,weak) id <ZLIntegralGoodsListCellDelgate>delegate;

@end

@implementation ZLIntegralGoodsListCell
...
#pragma mark - Lazy
- (ZLIntegralGoodsListUnitView *)leftBlockView {
    if (!_leftBlockView) {
        ZLIntegralGoodsListUnitView *leftBlockView = [[ZLIntegralGoodsListUnitView alloc] initWithFrame:CGRectMake(15.0, 15.0, (UIScreen.mainScreen.bounds.size.width - 60.0) / 2, 195.0)];
        [self.contentView addSubview:leftBlockView];
        _leftBlockView = leftBlockView;
    }
    return _leftBlockView;
}
- (ZLIntegralGoodsListUnitView *)rightBlockView {
    if (!_rightBlockView) {
        ZLIntegralGoodsListUnitView *rightBlockView = [[ZLIntegralGoodsListUnitView alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.leftBlockView.frame) + 30.0, 15.0, CGRectGetWidth(self.leftBlockView.frame), CGRectGetHeight(self.leftBlockView.frame))];
...
+ (instancetype)reuseCellWithTableView:(UITableView *)tableView IndexPath:(NSIndexPath *)indexPath Delegate:(id)delegate Model:(id)model {
    ZLIntegralGoodsListCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ZLIntegralGoodsListCell class])];
    if (!cell) {
        cell = [[self alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:NSStringFromClass([ZLIntegralGoodsListCell class])];
        ...
        cell.leftBlockView.click = ^{
            if (weakCell.delegate && [(UIView *)weakCell.delegate respondsToSelector:@selector(lookDetailWithModel:)]) {
                ...
                ZLIntegralGoodsListModel *infoModel = model;
    ...
    if (leftNum < infoModel.unitModels.count) {
        ZLIntegralGoodsListModel *rowModel = infoModel.unitModels[leftNum];
    ...
    if (rightNum < infoModel.unitModels.count) {
        ZLIntegralGoodsListModel *rowModel = infoModel.unitModels[rightNum];
