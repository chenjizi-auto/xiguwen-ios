//
//  ZLIntegralGoodsSureOrderCell.m
//  ProjectModules
//
...
@interface ZLIntegralGoodsSureOrderCell ()
...
@implementation ZLIntegralGoodsSureOrderCell
...
+ (instancetype)reuseCellWithTableView:(UITableView *)tableView IndexPath:(NSIndexPath *)indexPath Model:(ZLIntegralGoodsSureOrderModel *)model {
    ZLIntegralGoodsSureOrderCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([ZLIntegralGoodsSureOrderCell class])];
    if (!cell) {
        cell = [[self alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:NSStringFromClass([ZLIntegralGoodsSureOrderCell class])];
    }
    [cell.iconButton setBackgroundImageForState:UIControlStateNormal withURL:[NSURL URLWithString:model.goodsImageUrl] placeholderImage:[UIImage imageNamed:@"square_placeholder_ small"]];
    cell.titleLabel.text = model.goodsName;
    cell.priceLabel.text = model.goodsPrice;
    cell.numberLabel.text = model.goodsNumber;
