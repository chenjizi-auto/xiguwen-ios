...
public class MineExampleAdapter extends RecyclerView.Adapter<MineExampleAdapter.ViewHolder> {
    Context mContext;
    int type = 0;
    List<MyExampleBean> mDatas;

    com.jcodecraeer.xrecyclerview.OnItemClickListener itemClickListener;

    public MineExampleAdapter(Context mContext, List<MyExampleBean> datas, int type, com.jcodecraeer.xrecyclerview.OnItemClickListener itemClickListener) {
    ...
    public MineExampleAdapter(Context mContext, List<MyExampleBean> datas, int type) {
        this.mContext = mContext;
        this.mDatas = datas;
        this.type = type;
    }
    ...
    @Override
    public MineExampleAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.item_baojia_fragment, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(MineExampleAdapter.ViewHolder vh, int position) {
        MyExampleBean bean = mDatas.get(position);
        switch(type){
            case 0:
        ...
        vh.tvPrice.setText(Constans.RMB + bean.getWeddingexpenses());
        if (bean.getWeddingcover() == null) {
            GlideLoad.GlideLoadImg(mContext, R.mipmap.icon_placeholder, vh.ivImg);
        } else {
            GlideLoad.GlideLoadRoundedImg(bean.getWeddingcover(), vh.ivImg, 6);
        }
    }
