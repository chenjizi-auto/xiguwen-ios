...
public class PopArrowAdapter3 extends RecyclerView.Adapter<PopArrowAdapter3.ViewHolder> {
    Context mContext;
    List<GetCityBean> arrow;
    com.jcodecraeer.xrecyclerview.OnItemClickListener itemClickListener;
    int select_point=0;

    public PopArrowAdapter3(Context mContext, List<GetCityBean> arrow, com.jcodecraeer.xrecyclerview.OnItemClickListener itemClickListener) {
    ...
    @Override
    public PopArrowAdapter3.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.pop_item_arrow_list, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(final PopArrowAdapter3.ViewHolder vh, int position) {
        ...
        vh.tvSelectTxt.setText(arrow.get(position).getName());
    }
