...
public class TodayHaveAdapter extends RecyclerView.Adapter<TodayHaveAdapter.ViewHolder> {
    Context mContext;
    ...
    public TodayHaveAdapter(Context mContext) {
        this.mContext = mContext;
    }

    @Override
    public TodayHaveAdapter.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(mContext).inflate(R.layout.item_layout_today_add_team, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(TodayHaveAdapter.ViewHolder vh, int position) {
        vh.tvTime.setVisibility(View.GONE);
//        vh.tvWhen.setText("上午");
        CommunityDanEntity entity = mBens.get(position);
        vh.tvName.setText(entity.getNickname() + "");
        vh.tvTime.setText(entity.getCreate_ti() + "");
        String day;
