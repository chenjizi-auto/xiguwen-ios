...
import com.linzi.xiguwen.adapter.MineExampleAdapter;
...
public class MineExampleFragment extends BaseFragment {
    ...
    MineExampleAdapter mAdapter;

    private List<MyExampleBean> mDatas;

    private int mPage = 1;
    private int mRows = 15;

    public static MineExampleFragment newInstance (int FLAG) {
        Bundle args = new Bundle();
        args.putInt("status", FLAG);
        MineExampleFragment fragment=new MineExampleFragment();
    ...
    private void initViews(){
        ...
        mAdapter=new MineExampleAdapter(getActivity(), mDatas, mStatus, new com.jcodecraeer.xrecyclerview.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int postion) {
                Intent intent=new Intent(getActivity(),MineExampleDetailsActivity.class);
    ...
    private void requestNetData(final boolean isRefresh){
        ApiManager.getMyExampleList(isRefresh ? 1 : mPage, mRows, mStatus, new OnRequestFinish<BaseBean<List<MyExampleBean>>>() {
            @Override
            ...
            @Override
            public void onSuccess(BaseBean<List<MyExampleBean>> data) {
                if(isRefresh){
                    mDatas.clear();
                    mPage = 1;
                }
