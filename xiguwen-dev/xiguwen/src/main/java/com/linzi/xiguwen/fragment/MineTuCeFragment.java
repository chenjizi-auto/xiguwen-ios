...
import com.linzi.xiguwen.adapter.MineTuCeAdapter;
...
public class MineTuCeFragment extends BaseFragment {
    ...
    MineTuCeAdapter mAdapter;

    private List<AtlasBean> mDatas;

    private int mPage = 1;
    private int mRows = 15;


    public static MineTuCeFragment newInstance (int status) {
        Bundle args = new Bundle();
        args.putInt("status", status);
        MineTuCeFragment fragment=new MineTuCeFragment();
    ...
    private void initViews(){
        ...
        mAdapter=new MineTuCeAdapter(getActivity(), mDatas, new com.jcodecraeer.xrecyclerview.OnItemClickListener() {
            @Override
            public void onItemClick(View view, int postion) {
                Intent intent=new Intent(getActivity(),TuCeDetailsActivity.class);
    ...
    private void requestNetData(final boolean isRefresh){
        ApiManager.getAtlasList(isRefresh ? 1 : mPage, mRows, mStatus, new OnRequestFinish<BaseBean<List<AtlasBean>>>() {
            @Override
            ...
            @Override
            public void onSuccess(BaseBean<List<AtlasBean>> data) {
                if(isRefresh){
                    mDatas.clear();
                    mPage = 1;
                }
