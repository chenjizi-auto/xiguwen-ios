...
public class WeddingCartFragment extends BaseLazyFragment {
    ...
    public static Fragment create() {
        WeddingCartFragment fragment = new WeddingCartFragment();
        return fragment;
    }
    ...
    private void getData() {
        ApiManager.getWeddingCart(new OnRequestFinish<BaseBean<WeddingCartBean>>() {
            @Override
            public void onFinished() {
            }

            @Override
            public void onSuccess(BaseBean<WeddingCartBean> data) {
                ...
                baseAdapter.injectHolderDelegate(new CreateHolderDelegate<WeddingCartBean.TuijianBean>() {
                    @Override
                    protected int getLayoutRes() {
    ...
    private void dealBean(WeddingCartBean bean) {
        map = new HashMap<>();
        List<WeddingCartBean.DataBean> list;
        for (int i = 0; i < bean.getData().size(); i++) {
            WeddingCartBean.DataBean bean1 = bean.getData().get(i);
    ...
    private void isAllselect(boolean isClicked) {
        if (isClicked) {
            ...
            for (List<WeddingCartBean.DataBean> bean : map.values()) {
    ...
    class FatherItemHolder extends BaseViewHolder<Integer> {
        ...
        public class GoodsAdapter extends RecyclerView.Adapter<GoodsAdapter.VH> {
            ...
            public void onBindViewHolder(final VH vh, final int position) {
                ...
                vh.tvName.setText(list.get(position).getBaojia_name() + "");
                vh.tvTime.setText("");
//                //1全款 2订金
//                vh.tvTypePay.setVisibility(list.get(position).getPaytype() == 1 ? View.GONE : View.VISIBLE);
                vh.tvDanjia.setText(Constans.RMB + list.get(position).getPrice());

                if (list.get(position).getPaytype() == 2) {
                    vh.tvDingjin.setText(Constans.RMB + list.get(position).getPartprice());
                ...
                VH(View view) {
                    ...
                    ivImg.setOnClickListener(new View.OnClickListener() {
                        ...
                        public void onClick(View view) {
                            ...
                            intent.putExtra("offoer_id", list.get(getPosition()).getBaojia_id());
                            getActivity().startActivity(intent);
                        }
                    });
                }
    ...
    class GuessYouLikeHolder extends BaseViewHolder<WeddingCartBean.TuijianBean> {
        ...
        @Override
        protected void bindView(WeddingCartBean.TuijianBean tuijianBean) {
        ...
        private int getRecommendPosition(WeddingCartBean.TuijianBean tuijianBean) {
                ...
                for (int j = 0; j < bean.get(i).getGoods().size(); j++) {
                    if (bean.get(i).getGoods().get(j).isChecked() == true) {
                        if (bean.get(i).getGoods().get(j).getPaytype() == 1) {
                            ...
                            price = NumberUtil.add(price, NumberUtil.AmultiplyB(bean.get(i).getGoods().get(j).getPartprice(), bean.get(i).getGoods().get(j).getQuantity() + ""));
                        }
                    }
                }
            }
    ...
    //修改商品数量
    private void updateNum(final WeddingCartBean.DataBean.GoodsBean goodsBean, final TextView textView, int rec_id, final int num) {
        LoadDialog.showDialog(getActivity());
        ApiManager.updateWeddingCartNumber(rec_id, num + "", new OnRequestFinish<BaseBean<WeddingCartBean>>() {
            @Override
            public void onFinished() {
    ...
    //删除商品
    private void delGoods(final WeddingCartBean.DataBean.GoodsBean goodsBean) {
        final int rec_id = goodsBean.getRec_id();
        final int key = goodsBean.getStore_id();
        LoadDialog.showDialog(getActivity());
        ApiManager.removeWeddingCart(rec_id, new OnRequestFinish<BaseBean>() {
            @Override
            public void onFinished() {
                LoadDialog.CancelDialog();
            }
            ...
            public void onSuccess(BaseBean data) {
                if (data.getCode() == 0) {
                    List<WeddingCartBean.DataBean> list = map.get(key);
                    int size = list.size();
                    if (size == 1) {
                        getData();
                    } else if (size > 1) {
