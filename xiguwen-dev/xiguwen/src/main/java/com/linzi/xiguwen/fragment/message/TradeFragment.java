...
public class TradeFragment extends Fragment implements com.jcodecraeer.xrecyclerview.OnItemClickListener1 {
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.fragment_message_trade, container, false);
    ...
    private void initView() {
        ...
        mAdapter = new MessageTradeAdapter(getActivity(), this);
    ...
    public void setCallback(ChatListFragment.MessageDotChangeListener changeListener) {
    ...
    private void refresh(@NonNull final RefreshLayout refreshLayout, String page, String row) {
        ApiManager.messageTrade(page, row, new OnRequestSubscribe<BaseBean<MessageTradeBean>>() {
            @Override
            public void onSuccess(BaseBean<MessageTradeBean> data) {
...
//                    mAdapter.addMore(data.getData().getCont());
    ...
    public void onItemClick(View view, int postion, Object data) {
        MessageTradeBean.MessageTradeEntity entity = (MessageTradeBean.MessageTradeEntity) data;

        Intent intent1 = new Intent(getActivity(), NewOrderDetailsActivity.class);
        intent1.putExtra("order_id", entity.getSid());
        intent1.putExtra("intentType", entity.getShifoujiedan());
        if (entity.getStatus() == 100){
            intent1.putExtra("status", entity.getStatus());
        }
        startActivity(intent1);
//       List<String> trades= Preferences.getTradeIds();
//       if (trades!=null&&trades.size()>0&&trades.contains(messageTradeEntity.getId())){
//           trades.remove(messageTradeEntity.getId());
...
//       }
    ...
    private List<MessageTradeBean.MessageTradeEntity> getData(List<MessageTradeBean.MessageTradeEntity> entityList) {
        List<String> tradeIds = Preferences.getPushTradeIds();
        if (AppUtil.isEmpty(tradeIds) || AppUtil.isEmpty(entityList)) {
            return entityList;
        }

        for (MessageTradeBean.MessageTradeEntity entity : entityList) {
