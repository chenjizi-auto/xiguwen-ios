...
public class DongTaiFragment extends BaseLazyFragment {
    ...
    private void afterView(DongTaiBean bean, boolean isLoadMore) {
        if (isLoadMore) {
            mAdapter.injectHolderDelegate(new CreateHolderDelegate<DongTaiBean.DongtaiBean>() {
                @Override
    ...
    private void getData(final boolean isLoadMore) {
        if (isLoadMore) {
            ...
            page = 1;
            if (mAdapter != null)
                mAdapter.clearAllDelegate();
        }
       // LoadDialog.showDialog(getContext());
        ApiManager.getDynamic(shop_id + "", page + "", limit + "", new OnRequestFinish<BaseBean<DongTaiBean>>() {
            @Override
            ...
            public void onSuccess(BaseBean<DongTaiBean> data) {
                ...
                if (dongTaiBean.getDongtai() != null && dongTaiBean.getDongtai().size() > 0) {
                    if (isLoadMore) {
                        bean.getDongtai().addAll(dongTaiBean.getDongtai());
    ...
    //动态Holder
    class DongTaiHolder extends BaseViewHolder<DongTaiBean.DongtaiBean> {
        ...
        //删除动态
        private void delDongTai() {
            LoadDialog.showDialog(getActivity());
            ApiManager.delDontTai(id, new OnRequestFinish<BaseBean>() {
                @Override
                public void onFinished() {
                    LoadDialog.CancelDialog();
                }
        ...
        @Override
        protected void bindView(DongTaiBean.DongtaiBean dongtaiBean) {
            if ((int) SPUtil.get("userid", SPUtil.Type.INT) == dongtaiBean.getUserid()) {
...
//            });
            ...
            tvZhiwei.setText(dongtaiBean.getOccupation());
            ...
            grid_image.setItemImageClickListener(new ItemImageClickListener<DongTaiBean.DongtaiBean.PhotourlBean>() {

                @Override
                public void onItemImageClick(Context context, ImageView imageView, int index, List<DongTaiBean.DongtaiBean.PhotourlBean> list) {
         ...
         * @param list 图片集合
         */
        private void computeBoundsBackward(List<DongTaiBean.DongtaiBean.PhotourlBean> list) {
        ...
        private NineGridImageViewAdapter<DongTaiBean.DongtaiBean.PhotourlBean> mAdapter = new NineGridImageViewAdapter<DongTaiBean.DongtaiBean.PhotourlBean>() {
            @Override
            protected void onDisplayImage(Context context, ImageView imageView, DongTaiBean.DongtaiBean.PhotourlBean s) {
            ...
            @Override
            protected void onItemImageClick(Context context, ImageView imageView, int index, List<DongTaiBean.DongtaiBean.PhotourlBean> list) {
    ...
    private BaseAdapter createAdapter(DongTaiBean bean) {
        BaseAdapter baseAdapter = BaseAdapter.createBaseAdapter().
                ...
                .injectHolderDelegate(new CreateHolderDelegate<DongTaiBean.DongtaiBean>() {
