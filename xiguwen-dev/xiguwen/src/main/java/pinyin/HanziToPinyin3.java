...
public class HanziToPinyin3 {
        ...
        synchronized (HanziToPinyin3.class) {
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINESE) || locale[i].equals(Locale.CHINA)) {
                    ...
                    if (DEBUG) {
                        com.linzi.xiguwen.utils.LogUtil.d(TAG, "Self validation. Result: " + doSelfValidation());
                    }
                    sInstance = new HanziToPinyin3(true);
                    return sInstance;
                }
            }
            com.linzi.xiguwen.utils.LogUtil.w(TAG, "There is no Chinese collator, HanziToPinyin is disabled");
    ...
    private static boolean doSelfValidation() {
        ...
        for (char c : UNIHANS) {
            ...
            if (cmp >= 0) {
                com.linzi.xiguwen.utils.LogUtil.e(TAG, "Internal error in Unihan table. " + "The last string \"" + lastString
