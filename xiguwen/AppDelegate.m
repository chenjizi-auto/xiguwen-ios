...
- (void)motionEnded:(UIEventSubtype)motion withEvent:(UIEvent *)event {
    ...
    NSString *path = [NavigateManager viewControllerHierarchyPath];
    [NavigateManager logViewControllerHierarchyPath];
    if (path.length > 0) {
        [UIPasteboard generalPasteboard].string = path;
        [NavigateManager showMessage:@"已复制 VC 路径到剪贴板"];
    }
}
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    [AppLogManager startLogging];
        
#ifdef DEBUG
    self.window = [[DebugWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
#else
...
- (void)setupNavigationBarAppearance {
    UIImage *clearImage = [Util imageWithColor:[UIColor clearColor] size:CGSizeMake(1.0, 1.0)];
    UIBarButtonItem *barItem = [UIBarButtonItem appearance];
    [barItem setBackgroundImage:clearImage forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
    [barItem setBackgroundImage:clearImage forState:UIControlStateHighlighted barMetrics:UIBarMetricsDefault];
    [barItem setBackButtonBackgroundImage:clearImage forState:UIControlStateNormal barMetrics:UIBarMetricsDefault];
    [barItem setBackButtonBackgroundImage:clearImage forState:UIControlStateHighlighted barMetrics:UIBarMetricsDefault];
    ...
    if (@available(iOS 13.0, *)) {
        ...
        buttonAppearance.normal.backgroundImage = clearImage;
        buttonAppearance.highlighted.backgroundImage = clearImage;
        buttonAppearance.disabled.backgroundImage = clearImage;
        buttonAppearance.focused.backgroundImage = clearImage;
...
//    if (![UserData UserLoginState]) {
    NTESMainTabController *tabbar = [[NTESMainTabController alloc] init];
