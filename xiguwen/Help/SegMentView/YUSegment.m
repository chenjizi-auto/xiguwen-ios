...
- (instancetype)initWithImages:(NSArray <UIImage *> *)images selectedImages: (NSArray <UIImage *> *)selectedImages {
    ...
    if (self) {
        _internalImages = [images mutableCopy];
        _numberOfSegments = [images count];
        _style = YUSegmentStyleLine;
        [self commonInit];
        [self configureImages: images selectedImages: selectedImages];
    }
    return self;
}
...
- (void)configureImages: (NSArray <UIImage *> *) images selectedImages:(NSArray <UIImage *> *) selectedImages {
    ...
    for (int i = 0; i < _numberOfSegments; i++) {
        ...
        // Configure selected image view
        imageView = [self configureImageViewWithImage: selectedImages[i]];
        [self.imageViewsSelected addObject: imageView];
        [_containerSelected addSubview: imageView];
    }
    [self setupConstraintsWithSubviewsInContainer:_containerNormal];
...
- (void)setupConstraintsWithSubviewsInContainer:(UIView *)container {
    ...
    for (int i = 0; i < _numberOfSegments; i++) {
        [NSLayoutConstraint constraintWithItem:subviews[i] attribute:NSLayoutAttributeTop relatedBy:NSLayoutRelationEqual toItem:container attribute:NSLayoutAttributeTop multiplier:1.0 constant: 12.0].active = YES;
        
        [NSLayoutConstraint constraintWithItem:subviews[i] attribute:NSLayoutAttributeBottom relatedBy:NSLayoutRelationEqual toItem:container attribute:NSLayoutAttributeBottom multiplier:1.0 constant: -12.0].active = YES;
