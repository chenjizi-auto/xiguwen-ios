...
- (void)showItemsWithTags:(NSArray *)tags Titles:(NSArray *)titles ImageNames:(NSArray *)imageNames SuperView:(UIView *)superView MaxY:(CGFloat)maxY {
    CGFloat width = 70.0;
    CGFloat height = 70.0;
    CGFloat containerWidth = CGRectGetWidth(superView.frame) > 0.0 ? CGRectGetWidth(superView.frame) : UIScreen.mainScreen.bounds.size.width;
    NSInteger maxColumns = 4;
    CGFloat sideInset = (containerWidth - width * maxColumns) / 8.0;
    CGFloat columnSpacing = sideInset * 2.0;
    for (NSInteger index = 0; index < tags.count; index++) {
        NSInteger column = index % maxColumns;
        NSInteger row = index / maxColumns;
        CGFloat unitX = sideInset + (width + columnSpacing) * column;
        CGFloat unitY = maxY + height * row;
        UIView *unitView = [[UIView alloc] initWithFrame:CGRectMake(unitX, unitY, width, height)];
