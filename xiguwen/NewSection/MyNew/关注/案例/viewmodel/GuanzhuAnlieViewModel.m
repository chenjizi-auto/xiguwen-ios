...
@implementation GuanzhuAnlieViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
    ...
    [self.dataArray addObjectsFromArray:[AnliGuanzhuModel mj_objectArrayWithKeyValuesArray:object[@"anli"]]];
...
- (RACSubject *)cancelFocusItemSubject {
	
	if (!_cancelFocusItemSubject) _cancelFocusItemSubject = [RACSubject subject];
	
	return _cancelFocusItemSubject;
}
...
- (RACCommand *)refreshDataCommand {
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            @strongify(self);
            return [[RequestManager sharedManager] RACRequestUrl:URL_New_myguanzhu
                                                          method:POST
                                                          loding:@""
                                                             dic:input];
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    GuanzhuAnlieTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"GuanzhuAnlieTableViewCell"];
    cell.model = self.dataArray[indexPath.row];
	WeakSelf(self);
	[cell setOnDidcancelFocus:^{
		[weakSelf.cancelFocusItemSubject sendNext:self.dataArray[indexPath.row]];
	}];
    return  cell;
}
