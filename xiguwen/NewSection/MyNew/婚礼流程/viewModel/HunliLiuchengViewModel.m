...
@implementation HunliLiuchengViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
    ...
    [self.dataArray addObjectsFromArray:[LiuChengArray mj_objectArrayWithKeyValuesArray:object]];
}
...
- (RACCommand *)refreshDataCommand {
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            @strongify(self);
            return [[RequestManager sharedManager] RACRequestUrl:URL_liuCheng
                                                          method:POST
                                                          loding:@"加载中..."
                                                             dic:input];
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    [tableView registerNib:[UINib nibWithNibName:@"HunliLiuchengTableViewCell" bundle:nil] forCellReuseIdentifier:@"HunliLiuchengTableViewCell"];
    HunliLiuchengTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"HunliLiuchengTableViewCell"];
	...
	[[[cell.editBtn rac_signalForControlEvents:(UIControlEventTouchUpInside)] takeUntil:cell.rac_prepareForReuseSignal] subscribeNext:^(__kindof UIControl * _Nullable x) {
		@strongify(self);
		[self.editItemSubject sendNext:self.dataArray[indexPath.row]];
	}];
...
+ (NSDictionary *)objectClassInArray {
	return @{@"liuChengArray":[LiuChengArray class]};
}

@end

@implementation LiuChengArray
