//
//  ShangchengOrderViewController.m
//  BoYi
//
...
#import "ShangchengOrderViewController.h"
...
@interface ShangchengOrderViewController ()<UIScrollViewDelegate>{
    NSInteger p;
}

@property (weak, nonatomic) IBOutlet UITableView *table;
@property (strong,nonatomic) ShangchengOrderViewModel *viewModel;

@end

@implementation ShangchengOrderViewController
...
- (void)cellClick {
    ...
    [self.viewModel.secondSubject subscribeNext:^(id  _Nullable x) {
        ...
        if (model.status == 70 || model.status == 80) { //查看物流
            LookWuliuViewController *look = [[LookWuliuViewController alloc] init];
            look.id = model.order_id;
            if (model.goods.count > 0) {
                look.imageurl = model.goods[0].goods_image;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    CGFloat sectionHeaderHeight = 108; //这里是我的headerView和footerView的高度
    if (self.table.contentOffset.y <= sectionHeaderHeight && self.table.contentOffset.y>=0) {
        self.table.contentInset = UIEdgeInsetsMake(-self.table.contentOffset.y, 0, 0, 0);
    } else if (self.table.contentOffset.y >= sectionHeaderHeight) {
        self.table.contentInset = UIEdgeInsetsMake(-sectionHeaderHeight, 0, 0, 0);
    }
}
...
- (void)setupTableView {
    ...
    [self.table registerNib:[UINib nibWithNibName:@"ShangchengOrderTableViewCell" bundle:[NSBundle mainBundle]] forCellReuseIdentifier:@"ShangchengOrderTableViewCell"];
...
//初始化viewModel
- (ShangchengOrderViewModel *)viewModel {
    if (!_viewModel) {
        _viewModel = [[ShangchengOrderViewModel alloc] init];
    }
    return _viewModel;
}
