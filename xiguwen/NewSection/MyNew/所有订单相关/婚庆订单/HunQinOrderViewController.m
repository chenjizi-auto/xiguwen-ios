...
static NSInteger const kHunQinOrderPageSize = 10;
...
@property (assign, nonatomic) BOOL shouldResetDataOnNextResponse;
@property (assign, nonatomic) BOOL hasAppearedOnce;
...
- (void)viewDidLoad
{
    ...
    [self cellClick];
    [self setupTableView];
    [self reloadOrderDataWithoutRefreshUI];
        ...
        [ZLHTTPSessionManager requestDataWithUrlPath:@"http://www.xiguwen520.com/appapi/ordershq/paypartfinishorder_user" Params:dictM POST:YES ModelArray:nil HttpHeader:NO Results:^(ZLSessionManagerErrorState sessionErrorState, id responseObject) {
            if (!sessionErrorState) {
                if (![responseObject[@"code"] intValue]) {
                    [weakSelf reloadOrderDataWithoutRefreshUI];
                    return;
                }
...
- (NSMutableDictionary *)requestParametersForPage:(NSInteger)page {
    NSMutableDictionary *dictM = [NSMutableDictionary new];
...
- (void)ensureFooterIfNeeded {
    ...
    self.table.mj_footer = [MJRefreshBackNormalFooter footerWithRefreshingBlock:^{
        @strongify(self);
        p ++;
        self.shouldResetDataOnNextResponse = NO;
        [self.viewModel.refreshDataCommand execute:[self requestParametersForPage:p]];
    }];
    self.table.mj_footer.hidden = YES;
}

- (void)reloadOrderDataWithoutRefreshUI {
    p = 1;
    self.shouldResetDataOnNextResponse = YES;
    [self ensureFooterIfNeeded];
    [self.viewModel.refreshDataCommand execute:[self requestParametersForPage:p]];
}

- (void)updateBackgroundColorForCurrentDataState {
    BOOL hasData = self.viewModel.dataArray.count > 0;
    UIColor *backgroundColor = hasData ? [UIColor colorWithWhite:0.93 alpha:1.0] : UIColor.whiteColor;
...
- (void)setupTableView {
    ...
    [self.viewModel.refreshUISubject subscribeNext:^(id  _Nullable x) {
        ...
        @strongify(self);
        BOOL shouldResetData = self.table.mj_header.isRefreshing || self.shouldResetDataOnNextResponse;
        self.shouldResetDataOnNextResponse = NO;
        //数据处理
        ...
        NSInteger currentCount = self.viewModel.dataArray.count;
        self.table.mj_footer.hidden = currentCount == 0;
        [self updateBackgroundColorForCurrentDataState];
        NSArray *array = x[@"data"];
        // 无数据时隐藏底部 footer，避免空态下方仍显示“没有更多数据”
        if (currentCount == 0) {
            [self.table.mj_footer endRefreshing];
        } else if (array.count < kHunQinOrderPageSize) {
