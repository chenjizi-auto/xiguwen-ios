...
- (void)setupTextFields {
    ...
    self.nameTF.spellCheckingType = UITextSpellCheckingTypeNo;
    self.nameTF.autocapitalizationType = UITextAutocapitalizationTypeNone;
    self.nameTF.inputAccessoryView = nil;
    [self disableKeyboardAssistantForTextField:self.nameTF];

    self.cardTF.keyboardType = UIKeyboardTypeASCIICapable;
    self.cardTF.returnKeyType = UIReturnKeyDone;
    self.cardTF.autocorrectionType = UITextAutocorrectionTypeNo;
    self.cardTF.spellCheckingType = UITextSpellCheckingTypeNo;
    self.cardTF.autocapitalizationType = UITextAutocapitalizationTypeNone;
    self.cardTF.inputAccessoryView = nil;
    [self disableKeyboardAssistantForTextField:self.cardTF];
}

- (void)setupImageViews {
    [self applyRoundedFillStyleToImageView:self.frontImage];
    [self applyRoundedFillStyleToImageView:self.reverseImage];
    [self applyRoundedFillStyleToImageView:self.shouChiZhaoImageView];
    self.showChiZhaoView.layer.cornerRadius = 8.0;
    self.showChiZhaoView.layer.masksToBounds = YES;
}

- (void)applyRoundedFillStyleToImageView:(UIImageView *)imageView {
...
- (void)disableKeyboardAssistantForTextField:(UITextField *)textField {
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    NSCharacterSet *allowedCharacterSet = [NSCharacterSet characterSetWithCharactersInString:@"0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"];
    return [string stringByTrimmingCharactersInSet:allowedCharacterSet].length == 0;
}
