...
static CGFloat const kCartSectionHorizontalInset = 12.0;
...
static CGFloat ShopCarRecommendationItemHeight(void) {
    CGFloat width = ShopCarRecommendationItemWidth();
    return width * 7.0 / 12.0 + 90.0;
}

static CGFloat ShopCarRecommendationSectionHeight(NSInteger itemCount) {
    NSInteger rowCount = MAX((NSInteger)((itemCount + 1) / 2), 1);
    return 50.0 + rowCount * ShopCarRecommendationItemHeight() + MAX(rowCount - 1, 0) * kRecommendationSpacing;
}
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == self.dataArray.count || self.dataArray.count == 0) {
        //推荐
        return ShopCarRecommendationSectionHeight(self.tuiJianArray.count);
    }else {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    UIView *container = [[UIView alloc] initWithFrame:CGRectMake(0.0, 0.0, ScreenWidth, 44.0)];
    container.backgroundColor = [UIColor clearColor];
    GoodsListNewHeader *header = [[NSBundle mainBundle]loadNibNamed:@"GoodsListNewHeader" owner:nil options:nil].firstObject;
    header.frame = CGRectMake(kCartSectionHorizontalInset, kCartSectionTopInset, ScreenWidth - kCartSectionHorizontalInset * 2.0, 44.0);
