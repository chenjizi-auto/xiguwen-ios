...
@implementation SureDingdanNewSCViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
    ...
    [self.dataArray addObjectsFromArray:[CartlistSC mj_objectArrayWithKeyValuesArray:object[@"cartlist"]]];
    self.model = [sureDingdanSCmodel mj_objectWithKeyValues:object];
...
- (RACCommand *)refreshDataCommand {
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            NSString *url;
            if (self.type == 1) {
                url = [HOMEURL stringByAppendingString:@"appapi/orders/getordermoney"];
            }else {
                url = [HOMEURL stringByAppendingString:@"appapi/cart/confirm_orders"];
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    SureDingDanNewSCTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"SureDingDanNewSCTableViewCell"];
    cell.modelsc = self.dataArray[indexPath.section].goods[indexPath.row];
    return  cell;
}
