...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 0) {
        ...
        if (self.markType == 0) {
            if (indexPath.row > 0) {
                return [DongtaiTableViewCell cellHeightForModel:self.modelIndex.dynamiclist[indexPath.row - 1]
            ...
            if (indexPath.row == 0) {
                ...
                NSInteger rows = self.modelZuopin.chengyuan.count / 2 + self.modelZuopin.chengyuan.count % 2;
                return 143 * rows;
            }
        }else {
            if (indexPath.row == 0) {
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    ...
    if (section == 1) {
        ...
        header.frame = CGRectMake(0, 0, ScreenWidth, 88);
