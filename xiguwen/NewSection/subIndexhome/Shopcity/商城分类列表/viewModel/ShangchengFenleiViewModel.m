...
@implementation ShangchengFenleiViewModel
...
- (void)ConvertingToObject:(id)object isHeaderRefersh:(BOOL)isHeaderRefersh {
    ...
    [self.dataArray addObjectsFromArray:[Shangchengfenlei mj_objectArrayWithKeyValuesArray:object]];
...
- (RACCommand *)refreshDataCommand {
    ...
    if (!_refreshDataCommand) {
        ...
        _refreshDataCommand = [[RACCommand alloc] initWithSignalBlock:^RACSignal *(id input) {
            ...
            @strongify(self);
            return [[RequestManager sharedManager] RACRequestUrl:URL_New_shangchengfenlei
                                                          method:POST
                                                          loding:@"加载中..."
                                                             dic:input];
        }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    ShangchengFenleiTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ShangchengFenleiTableViewCell"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    for (int i = 0; i < self.dataArray.count; i++ ) {
        self.dataArray[i].isselete = NO;
    }
    self.dataArray[indexPath.row].isselete = YES;
