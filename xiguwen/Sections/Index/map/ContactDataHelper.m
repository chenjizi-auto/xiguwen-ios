...
+ (NSMutableArray *) getFriendListDataBy:(NSMutableArray *)array{
    ...
    NSArray *serializeArray = [(NSArray *)array sortedArrayUsingComparator:^NSComparisonResult(NewmapModel * obj1, NewmapModel * obj2) {//排序
        int i;
    ...
    for (NewmapModel *user in serializeArray) {
...
+ (NSMutableArray *)getFriendListSectionBy:(NSMutableArray *)array{
    ...
    for (NSArray *item in array) {
        NewmapModel *user = [item objectAtIndex:0];
        char c = [user.pinyin characterAtIndex:0];
        if (!isalpha(c)) {
            c = '#';
        }
