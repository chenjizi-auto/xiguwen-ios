...
- (void)pushAnimation:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    RDVTabBarController *tabbarController = fromViewController.rdv_tabBarController;
    if (!tabbarController && [fromViewController isKindOfClass:[UITabBarController class]]) {
        tabbarController = (RDVTabBarController *)fromViewController.tabBarController;
    }
    ...
    if ((toViewController.edgesForExtendedLayout & UIRectEdgeBottom) == 0) {
        ...
        CGFloat tabBarHeight = tabbarController ? tabbarController.tabBar.height : 0.0;
        CGRectDivide(frame, &slice, &remainder, tabBarHeight, CGRectMaxYEdge);
        frame = remainder;
    }
...
- (void)popAnimation:(id<UIViewControllerContextTransitioning>)transitionContext
{
    ...
    BOOL hidesBottomBar = [toViewController ntes_shouldHideTabBarInNavigationController:self.navigationController];
    
    RDVTabBar *tabbar = [NTESMainTabController instance].tabBar;
    UIView *containerView = [transitionContext containerView];
