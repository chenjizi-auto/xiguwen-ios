...
- (UIBarButtonItem *)customBarButtonItemWithView:(UIView *)customView {
...
- (void)addPopBackBtn {
    ...
    [backBtn addTarget:self action:@selector(popViewConDelay)forControlEvents:UIControlEventTouchUpInside];
    self.navigationItem.leftBarButtonItem = [self customBarButtonItemWithView:backBtn];
}
- (void)addRightBtnWithTitle:(NSString *)title image:(NSString *)image {
    ...
    self.navigationItem.rightBarButtonItem = [self customBarButtonItemWithView:backBtn];
}
...
- (void)popViewControllerAtLastIndex:(NSInteger)index {
    ...
    NSInteger targetIndex = count - index;
